...
File_Path = 'shop_analysis.csv'
Shop_Analysis = pd.read_csv(File_Path)
#Shop_Analysis_Clean = Shop_Analysis.dropna()
df = Shop_Analysis.copy()
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Home", "Daily Data Analysis", "Monthly Data Analysis", "Yearly Data Analysis", "Item Data Analysis"])
...
def adjusted_grams(row):
    if row['Quality'] == '15ပဲရည် ရွှေထည်' and row['Item'] == 'ရွှေထည်အရောင်းစာရင်း':
...
with tab2:
    ...
    Total_Weight.columns = ['Date','kyat_sum', 'kyat_mean', 'pel_sum','pel_mean','Yway_sum','Yway_mean']
    def tola (row):
...
with tab3:
    ...
    Total_MWeight = df.groupby(['Year','Month']).agg({'ကျပ်': ['sum','mean'], 'ပဲ' : ['sum', 'mean'],'ရွေး' : ['sum', 'mean']}).reset_index()
    Total_MWeight.columns = ['Year','Month','kyat_sum', 'kyat_mean', 'pel_sum','pel_mean','Yway_sum','Yway_mean']
    def tola (row):
        a = float(row['kyat_sum'])
        b = float(row['pel_sum'])
        c = float(row['Yway_sum'])
...
with tab4:
    ...
    Total_YWeight = df.groupby('Year').agg({'ကျပ်': ['sum','mean'], 'ပဲ' : ['sum', 'mean'],'ရွေး' : ['sum', 'mean']}).reset_index()
    Total_YWeight.columns = ['Year','kyat_sum', 'kyat_mean', 'pel_sum','pel_mean','Yway_sum','Yway_mean']
    def tola (row):
        a = float(row['kyat_sum'])
        b = float(row['pel_sum'])
        c = float(row['Yway_sum'])
